...
const Following=({following})=>{
  ...
  return (
    <div key={following._id} className={`flex flex-col  transition duration-150 ease-in-out ${dark?"bg-black hover:bg-[#232334]":"bg-white hover:bg-slate-100"} rounded-2xl left-2  p-2 mb-2 min-w-[75vw] cursor-pointer`} onClick={()=>{setFollowingDiv(false);navigate(`/people/${following.followable._id}`)}}>
...
const Followingsm = () => {
    ...
    return (
      <div className={`relative h-full flex sm:hidden flex-col min-w-[33vw] ml-2 items-center right-2 rounded-3xl p-2 border-2 ${dark?"border-slate-700 ":"border-slate-300"} scroll-smooth`}>
