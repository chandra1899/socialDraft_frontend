...
const PostProfile = ({user}) => {
  ...
  return (
    <div className='flex flex-row p-3 ml-2' >
     {user && <img src={`${user.photoLocal?`http://localhost:8000/photo/${user.photoLocal_path}?${Date.now()}`:`http://localhost:8000/api/user/userAvatar/${user._id}?${Date.now()}`}`} alt="logo" className='h-[40px] w-[40px] rounded-full mr-3 cursor-pointer' onClick={()=>{navigate(`/people/${user._id}`)}} />}
