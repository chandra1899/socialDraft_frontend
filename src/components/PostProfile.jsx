...
const PostProfile = ({user}) => {
  ...
  return (
    <div className='flex flex-row p-3 ml-2' >
      <img src={`${user.avatar?`http://localhost:8000/photo/${user.avatar}`:logo}`} alt="logo" className='h-[40px] w-[40px] rounded-full mr-3 cursor-pointer' onClick={()=>{navigate(`/people/${user._id}`)}} />
